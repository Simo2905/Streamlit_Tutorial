...
with st.form(key='sample_form'):
    ...
    st.subheader("Date and Inputs")
    date = st.date_input("Select your date of birth")
    time = st.time_input("Choose a preferred time")
    
    # Selectors
    st.subheader("Selectors")
    choice = st.radio("Chose an option", ['Option 1','Option 2','Option 3'])
    gender = st.selectbox("Select your gender", ['Male','Female'])
    slider_value = st.select_slider("Select a range", options=[1,2,3,4])
    ...
    submit_button = st.form_submit_button(label="Submit now!")
    
   # Streamlit salva i dati inseriti prima che rerun il programma 
